...
function doGet(e) {
  ...
  if (!autorizadoSaldos(params.token)) return jsonResponse({ error: 'forbidden' });
...
// ── Sesión firmada del SSO (tga_session): token = "usuario.exp.sig",
// sig = hex HMAC-SHA256("usuario.exp", app_config.tga_session_secret).
var USUARIOS_SALDOS = ['fngonzalez','fgonzalez','cgonzalez','mgerez','marianom','vreyna'];

// Secreto = app_config.tga_session_secret, copiado a mano en la Script Property
// TGA_SESSION_SECRET (este script no tiene key de Supabase ni permiso de UrlFetch).
function secretoSesionSaldos() {
  return PropertiesService.getScriptProperties().getProperty('TGA_SESSION_SECRET') || '';
}

function verificarSesionTGA_(tok) {
  tok = String(tok || '').trim();
  var i = tok.lastIndexOf('.'); if (i < 0) return null;
  var cuerpo = tok.slice(0, i), sig = tok.slice(i + 1).toLowerCase();
  var j = cuerpo.lastIndexOf('.'); if (j <= 0) return null;
  var exp = Number(cuerpo.slice(j + 1));
  if (!exp || exp < Math.floor(Date.now() / 1000)) return null;   // vencida
  var secret = secretoSesionSaldos(); if (!secret) return null;
  var hex = Utilities.computeHmacSha256Signature(cuerpo, secret)
